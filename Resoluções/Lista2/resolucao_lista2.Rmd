...
Mesma coisa, mas tentando melhorar a implementação para a melhor precisão que der
...
No modelo da lista 1, o resultado para (1, 1) dava quanto mesmo?
...
p_true <- ggplot(grid, aes(x1, x2, z = mu_true)) +
  geom_contour_filled() +
  labs(title = expression(mu(x[1], x[2]))) +
  theme_minimal()

p_hat <- ggplot(grid, aes(x1, x2, z = mu_hat)) +
  geom_contour_filled() +
  labs(title = expression(hat(mu)(x[1], x[2]))) +
  theme_minimal()
...
Obs - revisar pra ver se é isso mesmo.
