...
gradiente_rms_prop <- function(alpha, r, e, x1_init, x2_init){
  ...
  while (TRUE) {
    ...
    # calculo derivada parcial 1 e derivada parcial 2
    # passo alpha = 1 para reusar a mesma função, é mais fácil
    step <- grad_step(1, x1, x2)
    ...
    # em caso de erro/divergir, faço só as iterações máximas!
    # nesse caso deixei esse valor hardcoded
    if (qtd_exec > 10000) {
      break
    }
...
gradiente_adam <- function(alpha, r1, r2, e, x1_init, x2_init){
  ...
  while (TRUE) {
    ...
    # calculo derivada parcial 1 e derivada parcial 2
    # novamente vou economizar e usar a mesma função de antes com alpha = 1
    step <- grad_step(1, x1, x2)
